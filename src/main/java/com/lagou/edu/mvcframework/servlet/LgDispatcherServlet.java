...
//    private Map<String, List<Object>> security = new HashMap<>();    // 存放允许访问的id，id拼接成string
    @Override
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            // 前面是class的处理，从这里后面是method的处理 拼接成为 /demo/query
            // 获取方法
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                // Pattern.compile(url)把url转成正则表达式
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url));
                // 这里有url和方法 我在这里进行security比较好
                ...
                if(method.isAnnotationPresent(LagouSecurity.class)){
                    ...
                    String securityValue = methodAnnotation.value();

                    // 把值放到里面
                    handler.getSecurityList().add(securityValue);
        ...
        try{
            ...
            for (int i = 0; i < classNames.size(); i++) {
                ...
                if(aClass.isAnnotationPresent(LagouController.class)) {
                    ...
                    // 如果指定了id，就以指定的为准
                    // @LagouService("demoService")
                    if(!"".equals(beanName.trim())) {  // 空串判断 trim方法是去掉前后的空格
                    ...
                    // 没有dao层都 不让dao都应该是和service都一样处理
...
////                    String beanValue = annotation.value();    // jyo
////                    String beanName = aClass.getSimpleName();
//
//
//                    // 这里需要存的东西有 controller方法名，value值，
//                    // 可以用一个value对应一个list 放在map里？
//                    // 所以我需要得到方法
//
////                    for (int i = 0; i < methods.length; i++) {
////                        Method method = methods[i];
...
//                        // 没有标Security的不处理
//                        if(!method.isAnnotationPresent(LagouSecurity.class)) {continue;}
//
//                        // 获取注解value值
//                        // @LagouSecurity("jyo")
...
//                }
    ...
    // 扫描类 把扫描到到bean都放入classNameList中
...
//        method.invoke() //
        ...
        // 找到权限使用者
//        String paraValue = req.getParameterMap().get("name").toString();

        String paraValue = paraValues[2].toString();

        for (String userName : handler.getSecurityList()) {


            if ( paraValue.equals(userName) ){
                ...
                resp.getWriter().write("You do not have permission to login");
                return;
            }
        }
